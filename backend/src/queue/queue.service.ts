...
export class QueueService implements OnModuleInit, OnModuleDestroy {
  ...
  onModuleInit() {
    ...
    this.redisSub.psubscribe('ai_result_*', (err, count) => {
      if (err) {
        this.logger.error('Failed to subscribe to AI results', err);
      } else {
        this.logger.log(`Subscribed to AI results channel pattern. Count: ${count}`);
      }
    });
    ...
    this.redisSub.on('pmessage', (pattern, channel, message) => {
      ...
      if (req) {
        try {
          ...
          req.reject('Failed to parse Python result');
        } finally {
  ...
  async dispatchPredictionJob(symbol: string): Promise<any> {
    const jobId = uuidv4();
    ...
    return new Promise((resolve, reject) => {
      ...
      const timeout = setTimeout(() => {
        if (this.pendingRequests.has(jobId)) {
          this.pendingRequests.delete(jobId);
          reject(new Error('AI Prediction Timeout - Python worker may be down or busy'));
        }
      }, 30000);
      ...
      // Push to Redis List
      this.redisMain.lpush('ai_tasks', JSON.stringify(task)).catch(err => {
